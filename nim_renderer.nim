...
import
  compiler/lexer, compiler/options, compiler/idents, strutils, nim_ast, nim_types, compiler/msgs
...
proc renderDefinitionName*(s: PSym, noQuotes = false): string =
  ...
  let x = s.name.s
  if noQuotes or (x[0] in SymStartChars and not nim_renderer.isKeyword(s.name)):
    result = x
  else:
    result = '`' & x & '`'
...
proc gsub(g: var TSrcGen, n: PNode, c: TContext) =
  ...
  of nkPrefix:
    ...
    if n.len > 1:
      ...
      if n[1].kind == nkPrefix or (opr != nil and nim_renderer.isKeyword(opr)):
        put(g, tkSpaces, Space)
